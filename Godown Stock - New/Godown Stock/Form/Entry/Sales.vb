...
Public Class Sales
    ...
    Private Sub Sales_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        SalesDate = DateTime.Now.ToString("dd/MM/yyyy")
        Datesales.Text = SalesDate

        InitializeDataGridView()
        BillNoTxt.Text = GetNextBillNumber()
        BillNoTxt.ReadOnly = True
        Totamttxt.ReadOnly = True
        LoadFinancialPeriod()
        If Not String.IsNullOrEmpty(AlterBillNo) Then
            LoadSalesEntry(AlterBillNo)
        End If
    End Sub
    ...
    Private Sub ItemForm_Shown(sender As Object, e As EventArgs) Handles Me.Shown
        Partyname.Focus()
    End Sub
    Private Sub Control_KeyDown(sender As Object, e As KeyEventArgs) Handles Partyname.KeyDown, ManualBillnoText.KeyDown, Datesales.KeyDown, Itembox.KeyDown, qtytxt.KeyDown, ratetxt.KeyDown, Addbtn.KeyDown, Totamttxt.KeyDown, RemarksTextBox.KeyDown, Savebtn.KeyDown

        If (sender Is Partyname Or sender Is Itembox) AndAlso KryptonListBox1.Visible Then
    ...
    Private Function GetNextBillNumber() As String
        ...
        Dim query As String = "SELECT ISNULL(Vt_Prefix, '') + CAST(ISNULL(Vt_Billno, 0) AS VARCHAR) + ISNULL(Vt_Suffix, '') AS Vt_FullBillNo FROM v_table WHERE Vt_Name = 'Sales'"
    ...
    Private Sub LoadAutoCompleteData()
        ...
        DTParty = LoadDataTable("SELECT Partyname FROM Ledger_Table WHERE UNDER = 'Customer' AND Active = 0")
    End Sub
    ...
    End Function
    Private Function GetCurrentStock(itemName As String) As Decimal
        Dim query As String = "
        SELECT 
        ...
        WHERE i.Itemname = @Itemname AND i.Active = 0
    ...
    Private Sub Addbtn_Click(sender As Object, e As EventArgs) Handles Addbtn.Click
        Dim itemName As String = Itembox.Text.Trim()

        If String.IsNullOrEmpty(itemName) OrElse Not IsValidInput(Itembox, DTItem) Then
            MessageBox.Show("Please select a valid item.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        Dim qtyValue As Decimal
        If Not Decimal.TryParse(qtytxt.Text, qtyValue) Then
            MessageBox.Show("Please enter a valid quantity.", "Validation", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        Dim stock As Decimal = GetCurrentStock(itemName)
        If qtyValue > stock Then
            MessageBox.Show($"Only {stock} units of '{itemName}' are available in stock.", "Insufficient Stock", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        If String.IsNullOrWhiteSpace(ratetxt.Text) OrElse String.IsNullOrWhiteSpace(Totamttxt.Text) Then
    ...
    Private Sub Savebtn_Click(sender As Object, e As EventArgs) Handles Savebtn.Click
        ...
        Dim totalAmount As Decimal = 0
        Dim remarks As String = RemarksTextBox.Text.Trim()
        Dim manualBillNo As String = ManualBillnoText.Text.Trim()
        ...
        If Not DateTime.TryParseExact(Datesales.Text, "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, parsedDate) Then
            MessageBox.Show("Invalid date format. Please use dd/MM/yyyy.", "Invalid Date", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        If String.IsNullOrWhiteSpace(manualBillNo) Then
        ...
        For Each row As DataGridViewRow In Guna2DataGridView1.Rows
            If Not row.IsNewRow Then
                Dim cellValue = row.Cells("TotalAmount").Value
                If cellValue IsNot Nothing AndAlso Decimal.TryParse(cellValue.ToString(), totalAmount) Then
                    totalAmount += Convert.ToDecimal(cellValue)
                End If
            End If
        Next
        ...
        Using sqlconnect As SqlConnection = Tools.GetConnection()
            Try
                ...
                Dim result As DialogResult = MessageBox.Show("Do you want to save?", "Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
                ...
                If isUpdate Then
                    Using getCmd As New SqlCommand("SELECT item_id FROM Sales_table WHERE Bill_No = @Bill_No", sqlconnect, transaction)
                        getCmd.Parameters.Add("@Bill_No", SqlDbType.VarChar).Value = billNo
                ...
                For Each row As DataGridViewRow In Guna2DataGridView1.Rows
                    If Not row.IsNewRow Then
                        ...
                        Dim cmdTextStock As String = If(isUpdate,
                        "IF EXISTS (SELECT 1 FROM Stock_table WHERE Bill_No = @Bill_No AND item_id = @item_id)
                         ...
                         ELSE
                            ...
                            VALUES (@Bill_No, @Stock_date, @ledger_id, @item_id, @Itemname, @quantity, @Rate, @Total_Amount, 2, @UserID)",
                        "INSERT INTO Stock_table (Bill_No, Stock_date, ledger_id, item_id, Itemname, quantity, Rate, Total_Amount, EntryType, UserID)
                            VALUES (@Bill_No, @Stock_date, @ledger_id, @item_id, @Itemname, @quantity, @Rate, @Total_Amount, 2, @UserID)")

                        Using cmd As New SqlCommand(cmdTextStock, sqlconnect, transaction)
                        ...
                        Dim cmdTextSales As String = If(isUpdate,
                        "IF EXISTS (SELECT 1 FROM Sales_table WHERE Bill_No = @Bill_No AND item_id = @item_id)
                            UPDATE Sales_table SET ledger_id = @ledger_id, Partyname = @Partyname, Manual_BillNo = @Manual_BillNo,
                                sale_date = @sale_date, Itemname = @Itemname, quantity = @quantity, Rate = @Rate,
                                Total_Amount = @Total_Amount, Remarks = @Remarks, UserID = @UserID
                            WHERE Bill_No = @Bill_No AND item_id = @item_id
                         ELSE
                            INSERT INTO Sales_table (ledger_id, Partyname, Bill_No, Manual_BillNo, sale_date, item_id, Itemname, quantity, Rate, Total_Amount, Cancel, EntryType, Remarks, UserID)
                            VALUES (@ledger_id, @Partyname, @Bill_No, @Manual_BillNo, @sale_date, @item_id, @Itemname, @quantity, @Rate, @Total_Amount, 0, 2, @Remarks, @UserID)",
                        "INSERT INTO Sales_table (ledger_id, Partyname, Bill_No, Manual_BillNo, sale_date, item_id, Itemname, quantity, Rate, Total_Amount, Cancel, EntryType, Remarks, UserID)
                            VALUES (@ledger_id, @Partyname, @Bill_No, @Manual_BillNo, @sale_date, @item_id, @Itemname, @quantity, @Rate, @Total_Amount, 0, 2, @Remarks, @UserID)")

                        Using cmd As New SqlCommand(cmdTextSales, sqlconnect, transaction)
                            cmd.Parameters.AddWithValue("@ledger_id", ledgerId)
                            cmd.Parameters.AddWithValue("@Partyname", Partyname.Text)
                            cmd.Parameters.AddWithValue("@Bill_No", billNo)
                            cmd.Parameters.AddWithValue("@Manual_BillNo", manualBillNo)
                            cmd.Parameters.AddWithValue("@sale_date", parsedDate)
                            cmd.Parameters.AddWithValue("@item_id", itemId)
                            cmd.Parameters.AddWithValue("@Itemname", itemName)
                            cmd.Parameters.AddWithValue("@quantity", quantity)
                            cmd.Parameters.AddWithValue("@Rate", rate)
                ...
                ' Delete removed items if updating
                If isUpdate Then
                    ...
                    For Each deletedItemId In deletedItemIds
                        ...
                        Using delCmd As New SqlCommand("DELETE FROM Sales_table WHERE Bill_No = @Bill_No AND item_id = @item_id", sqlconnect, transaction)
                            delCmd.Parameters.AddWithValue("@Bill_No", billNo)
                            delCmd.Parameters.AddWithValue("@item_id", deletedItemId)
                            delCmd.ExecuteNonQuery()
                        End Using
                ...
                ' Update bill number if inserting
                If Not isUpdate Then
                    Using cmd As New SqlCommand("UPDATE v_table SET Vt_Billno = Vt_Billno + 1 WHERE Vt_Name = 'Sales'", sqlconnect, transaction)
                        cmd.ExecuteNonQuery()
                    End Using
                End If
    ...
    Public Sub LoadSalesEntry(ByVal billNo As String)
        Try
            Using sqlconnect As SqlConnection = Tools.GetConnection()
                ...
                Guna2DataGridView1.Rows.Clear()
                Using cmd As New SqlCommand("SELECT Itemname,quantity,Rate,Total_Amount FROM Sales_table WHERE Bill_No = @BillNo and Cancel = 0", sqlconnect)
                    cmd.Parameters.AddWithValue("@BillNo", billNo)
                    Using reader As SqlDataReader = cmd.ExecuteReader()
                        While reader.Read()
            ...
            End Using
        Catch ex As Exception
            MessageBox.Show("Error loading sales entry: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning)
        End Try
    End Sub
End Class