<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="UnitMaster.aspx.cs" Inherits="BKBilling.Forms.Master.UnitMaster" %>

<!DOCTYPE html>
<html>
<head runat="server">
    <title>Unit Master | BK Softwares</title>
    ...
    <style>
        ...
        .card-custom { 
            ...
            box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); margin: 0 auto; overflow: hidden;
        }
        ...
        .section-title { 
            font-size: 0.75rem; font-weight: 800; color: #6366f1; text-transform: uppercase; 
            letter-spacing: 0.1em; margin-bottom: 25px; display: flex; align-items: center;
        }
        ...
        .modern-switch {
            display: flex; align-items: center; background: #f8fafc; padding: 10px 15px 10px 45px; 
            border-radius: 12px; border: 1px solid #e2e8f0; position: relative; min-height: 45px; width: fit-content; min-width: 280px;
        }
        .modern-switch span { position: absolute; left: 10px; display: flex; align-items: center; }
        .modern-switch .form-check-input { width: 2.5em !important; height: 1.25em !important; cursor: pointer; margin: 0 !important; appearance: none; -webkit-appearance: none; background-color: #cbd5e1; border: none !important; }
...
<body>
    <form id="form1" runat="server">
        ...
        <div class="container-fluid">
            <!-- VIEW 1: UNIT DIRECTORY -->
            <asp:Panel ID="pnlList" runat="server">
                <div class="card-custom">
                    <div class="form-header d-flex justify-content-between align-items-center">
                        <div>
                            <h3 class="fw-bold m-0 text-dark">Unit Directory</h3>
                            <p class="text-muted small m-0">Manage units and decimal precision for items.</p>
                        </div>
                        <div class="d-flex gap-3">
                            <div class="input-group input-group-sm" style="width: 280px;">
                                <asp:TextBox ID="txtSearch" runat="server" CssClass="form-control" placeholder="Search units..." AutoPostBack="true" OnTextChanged="txtSearch_TextChanged" />
                            ...
                            <asp:LinkButton ID="btnOpenCreate" runat="server" CssClass="btn btn-primary px-4 fw-bold shadow-sm" OnClick="btnOpenCreate_Click">
                                <i class="fas fa-plus-circle me-2"></i>New Unit
                            </asp:LinkButton>
                        </div>
                    </div>
                    
                    <asp:GridView ID="gvUnits" runat="server" AutoGenerateColumns="false" CssClass="table gv-style" 
                        GridLines="None" DataKeyNames="Unit_Sno" OnRowCommand="gvUnits_RowCommand">
                        <Columns>
                            <asp:BoundField DataField="Unit_Sno" HeaderText="ID" ItemStyle-Width="100px" ItemStyle-CssClass="text-muted small" />
                            <asp:BoundField DataField="Unit_Name" HeaderText="Full Name" ItemStyle-CssClass="fw-bold text-dark" />
                            <asp:BoundField DataField="Unit_Sname" HeaderText="Symbol" ItemStyle-CssClass="text-indigo fw-bold" />
                            <asp:BoundField DataField="Decimal_Places" HeaderText="Decimals" />
                            ...
                            <asp:TemplateField HeaderText="Actions" ItemStyle-CssClass="text-end pe-4">
                                <ItemTemplate>
                                    <asp:LinkButton ID="btnEdit" runat="server" CssClass="btn btn-sm btn-outline-primary fw-bold" 
                                        CommandName="EditUnit" CommandArgument='<%# Eval("Unit_Sno") %>'>
            ...
            <asp:Panel ID="pnlForm" runat="server" Visible="false">
                <div class="card-custom">
                    ...
                    <div class="form-body">
                        <div class="row g-5">
                            <div class="col-md-7">
                                <p class="section-title">Identity & Precision</p>
                                
                                <div class="mb-4">
                                    <label class="form-label">Unit Full Name <span class="text-danger">*</span></label>
                                    <asp:TextBox ID="txtUnitName" runat="server" CssClass="form-control form-control-sm" placeholder="e.g. KILOGRAMS"></asp:TextBox>
                                    <asp:RequiredFieldValidator ID="rfv1" runat="server" ControlToValidate="txtUnitName" ErrorMessage="Full name required" CssClass="text-danger small" ValidationGroup="save" Display="Dynamic" />
                                </div>

                                <div class="row">
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label">Symbol / Short Name <span class="text-danger">*</span></label>
                                        <asp:TextBox ID="txtSymbol" runat="server" CssClass="form-control form-control-sm" placeholder="e.g. KG"></asp:TextBox>
                                        <asp:RequiredFieldValidator ID="rfv2" runat="server" ControlToValidate="txtSymbol" ErrorMessage="Symbol required" CssClass="text-danger small" ValidationGroup="save" Display="Dynamic" />
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label">Decimal Places</label>
                                        <asp:DropDownList ID="ddlDecimals" runat="server" CssClass="form-select form-select-sm">
                            ...
                            <div class="col-md-5 border-start ps-md-5">
                                ...
                                <div class="modern-switch form-check form-switch">
                                    ...
                                    <label class="form-check-label" for="<%= chkIsActive.ClientID %>">Is this unit currently active?</label>
                                </div>
                                <div class="mt-4 p-3 bg-light rounded border-start border-4 border-indigo small text-muted">
                                    <i class="fas fa-info-circle me-1"></i> Decimal places control how quantities are displayed and calculated in invoices.
                                </div>
                            </div>
                        </div>
                    </div>
